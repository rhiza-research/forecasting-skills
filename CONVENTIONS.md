...
| Window anchor | `--anchor` | `today` \| `YYYY-MM-DD` | Upper bound used when resolving a `--last` window's end. `today` is the default. The end is the latest available data on or before the anchor (the production lag is whatever the data dictates), so the caller never hardcodes a lag. Only valid alongside `--last`; supplying it with `--start`/`--end` is an error, not a silent no-op. |
