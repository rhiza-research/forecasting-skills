...
### Arguments
- `--date` — forecast init date. `YYYY-MM-DD` or `latest` (newest accessible S2S init (skips embargoed inits via ECDS probe)). Offsets like `latest-3w` / `now` are not accepted (decorator exits 2). Prefer recording the resolved absolute init in provenance.
