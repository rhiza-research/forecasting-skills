...
```
uv run scripts/resolve.py <CODE> [--geojson PATH]
```

Print just the bbox:

```bash
uv run scripts/resolve.py KEN
...
```bash
uv run scripts/resolve.py KEN --geojson /tmp/ken.json
```
...
BBOX=$(uv run scripts/resolve.py KEN)
