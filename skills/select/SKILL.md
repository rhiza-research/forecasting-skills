...
dataset, either by integer position (`--index`) or by coordinate value
(`--value`), and writes a new dataset. A single selection collapses the
...
through unchanged. Works on gridded and station datasets alike.
