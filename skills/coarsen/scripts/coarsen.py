...
def coarsen(ds, variable, target_resolution, offset):
    ...
    for axis in (lat_dim, lon_dim):
        spacing = float(abs(np.median(np.diff(np.asarray(ds[axis].values)))))
        if target_resolution < spacing:
            raise UsageError(
                f"--target-resolution {target_resolution}° finer than input; use downscale"
            )
    target = xr.Dataset(
        coords={
            lat_dim: (lat_dim, _target_axis(ds[lat_dim].values, target_resolution, offset), dict(ds[lat_dim].attrs)),
            lon_dim: (lon_dim, _target_axis(ds[lon_dim].values, target_resolution, offset), dict(ds[lon_dim].attrs)),
        }
    )
