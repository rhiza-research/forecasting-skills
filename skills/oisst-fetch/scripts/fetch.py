...
def main() -> None:
    ...
    def _latest() -> date:
        ...
        for year in (today.year, today.year - 1):
            ...
            except Exception as exc:  # noqa: BLE001 -- try previous year, else surface
                last_err = exc
                continue
        print(
            f"Error: could not resolve 'latest' from OISST OPeNDAP ({last_err}).", file=sys.stderr
        )
        sys.exit(1)
    ...
    for year in years:
        try:
            dy = _open_year(year)
        except Exception as exc:  # noqa: BLE001 -- a missing/unpublished year is a clear error
            print(f"Error: could not open OISST year {year} ({exc}).", file=sys.stderr)
            sys.exit(1)
        with dy:
