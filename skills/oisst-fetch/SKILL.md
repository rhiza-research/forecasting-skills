...
bounding box and time range, maps it onto the weather-skills standard dataset shape, and
...
### Arguments
- `--start`, `--end` — inclusive date range. Each value is `YYYY-MM-DD` or `latest` (newest day present in the served OISST year files). Both ends inclusive (see CONVENTIONS date grammar).
