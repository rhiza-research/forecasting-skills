...
dataset analysis shape, and writes a consolidated Zarr store. The store is a
...
### Arguments
- `--start`, `--end` — inclusive date range. Each value is `YYYY-MM-DD` or `latest` (newest time available in the ARCO ERA5 store). Both ends inclusive (see CONVENTIONS date grammar).
