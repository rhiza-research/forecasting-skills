...
description: Roll up a weather-skills standard dataset Zarr along its time axis (or forecast step axis) into fixed windows (daily, weekly, dekadal, monthly) with mean/min/max. Stamps aggregation_period and CF cell_methods. Rates in and rates out — use convert-to-totals for period amounts.
license: MIT
...
- For period **totals** (`mm`), run `convert-to-totals` afterward.
...
- `aggregation_period` — pint duration for the window (`1 day`, `7 day`,
  `1 dekad`, `1 month`) used later by `convert-to-totals`.
...
History entry records period, method, variables, and paths as usual.
